...
def test_create_account_duplicate_bank_account_no(client):
    ...
    client.post("/accounts/add", json=account_data_1) # This should succeed and be cleaned up by fixture
...
def test_create_account_with_negative_balance(client):
    ...
    assert any("Balance shouldn't be less than 0" in error.get("msg", "") for error in response.json().get("detail", []))